...
; bytes for it, and finally creating a symbol at the top. The stacks grows
; downwards on x86. The stack is in its own section so it can be marked nobits,
; which means the kernel file is smaller because it does not contain an
; uninitialized stack. The stack on x86 must be 16-byte aligned according to the
; System V ABI standard and de-facot extensions. The compiler will assume the
...
_start:
    ...
    mov esp, stack_top                              ; point esp to start of the stack (end of memory area)
